...
2. `a` 키 → 이름 / 호스트(IP 또는 SSH alias) / 사용자명 / 개인키 경로 입력
3. `d` 키로 삭제
...
`local` 항목은 저장하지 않아도 로드 시 자동으로 맨 앞에 추가됩니다.
...
- **시크릿은 저장소에 없습니다.** SSH 키는 런타임에 `~/.ssh`에서, OAuth 토큰은 Keychain/`~/.claude/.credentials.json`에서 읽습니다. 설정도 저장소 밖(`~/.agent-view/`)에 있습니다.
